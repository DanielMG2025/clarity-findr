...
export function FinancingSimulator({ amountMin, amountMax }: { amountMin: number; amountMax: number }) {
  ...
  const apr = 0.08; // tasa orientativa
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-lg font-bold">Simulador de financiación</h3>
      </div>
      <p className="text-sm text-muted-foreground mb-5">Cuota mensual estimada con un TAE orientativo del 8%. No es una oferta financiera.</p>

      <div className="grid sm:grid-cols-2 gap-6">
        <div>
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">Importe a financiar</Label>
        ...
        <div>
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">Plazo (meses)</Label>
          <div className="text-2xl font-bold tabular-nums my-1">{months} meses</div>
      ...
      <div className="mt-6 p-4 rounded-xl bg-primary-soft/40 border border-primary/15">
        <div className="text-xs text-muted-foreground">Cuota mensual estimada</div>
        <div className="text-3xl font-bold tabular-nums text-primary">€{monthly.toLocaleString()}/mes</div>
        <div className="text-xs text-muted-foreground mt-1 tabular-nums">
          Rango entre €{monthlyLow.toLocaleString()} y €{monthlyHigh.toLocaleString()}/mes según escenario
        </div>
      </div>
    </Card>
  );
